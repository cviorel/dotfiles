...
function Open-HistoryFile {
    $editor = $editor -or 'notepad'  # Default to 'notepad' if $editor is not set
    $historyPath = (Get-PSReadLineOption).HistorySavePath
...
#region Git Prompt
function Get-EmojiChar([string]$U) {
    $EmojiIcon = [System.Convert]::toInt32($U, 16)
    return [System.Char]::ConvertFromUtf32($EmojiIcon)
}

function Write-GitStatus {
    $status = git status -b --porcelain=v2 2>$null
    if ($LASTEXITCODE -eq 0) {
        $branchMatch = $status | Select-String "^# branch.head (.+)$"
        $branch = if ($branchMatch) { $branchMatch.Matches[0].Groups[1].Value } else { "unknown" }

        $aheadMatch = $status | Select-String "^# branch.ab \+(\d+) -\d+$"
        $ahead = if ($aheadMatch) { [int]$aheadMatch.Matches[0].Groups[1].Value } else { 0 }

        $behindMatch = $status | Select-String "^# branch.ab \+\d+ -(\d+)$"
        $behind = if ($behindMatch) { [int]$behindMatch.Matches[0].Groups[1].Value } else { 0 }

        $staged = @($status | Select-String "^1 ").Count
        $modified = @($status | Select-String "^1 .M").Count
        $untracked = @($status | Select-String "^\?").Count

        $gitInfo = " [$(Get-EmojiChar '1F33F')$branch"  # Leaf emoji
        if ($ahead -gt 0) { $gitInfo += " $(Get-EmojiChar '2B06')$ahead" }  # Up arrow emoji
        if ($behind -gt 0) { $gitInfo += " $(Get-EmojiChar '2B07')$behind" }  # Down arrow emoji
        if ($staged -gt 0) { $gitInfo += " $(Get-EmojiChar '2705')$staged" }  # Check mark emoji
        if ($modified -gt 0) { $gitInfo += " $(Get-EmojiChar '1F527')$modified" }  # Wrench emoji
        if ($untracked -gt 0) { $gitInfo += " $(Get-EmojiChar '2753')$untracked" }  # Question mark emoji
        $gitInfo += "]"
...
function Prompt {
    ...
    $promptString = "$(Get-EmojiChar '1F680') PS $location"  # Rocket emoji
