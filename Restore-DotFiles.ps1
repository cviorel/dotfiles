    ...
    try {
        if ($item.Application -match 'psCoreProfile|psProfile|VSCode|azDataStudio') {
            if (!(Test-Path -Path $item.Source)) {
                ...
                New-Item -Path $item.Source -ItemType File -Force -ErrorAction Stop
            }
        }
        ...
        if (Test-Path -Path $item.Source) {
            ...
            Copy-Item -Path "$gitRepo\$($item.Application)\$fileName" -Destination "$($item.Source)" -Force -ErrorAction Stop
        }
        ...
        if ($item.Application -eq 'VSCode') {
            ...
            if (Test-Path -Path "$gitRepo\$($item.Application)\snippets") {
                ...
                Copy-Item -Path "$gitRepo\$($item.Application)\snippets" -Destination "$vscodeRoot" -Recurse -Force -ErrorAction Stop
            }
